...
if(isset($_POST['add_cont'])){ 
	...
	if(add_cont($_POST['product'],$_POST['size'], $name)==1){
		redirect('../../index', 'ok=true');
	}
}
if(isset($_POST['edit_user_save'])){
	if(edit_user($_GET['id'], $_POST['edit_user_login'], $_POST['password'], $_POST['edit_user_name'], $_POST['edit_user_surname'])==1){
		redirect('../../admin', 'ok=true');
	}
}
if(isset($_POST['myreq_edit_save'])){
	if(!isset($_SESSION['admin'])){$name=$_SESSION['user'];}else {$name=$_SESSION['admin'];}
	if(edit_cont($name, $_POST['myreq_item_name'], $_POST['myreq_item_amount'], $_GET['id'])==1){
	redirect('../../index', 'ok=true');
	}
}
?>