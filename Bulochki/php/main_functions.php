...
function add_contact($name,$email,$tel, $product){
	global $conn;
	$conn->query("INSERT INTO contacts(id, name, email, tel, product, svaz) VALUES (NULL, '$name','$email','$tel', '$product', 0)");
	return 1;
}
function edit_contact($name='', $email='', $tel='',$product='',$svaz=0, $id){
	global $conn;
		$conn->query("
		UPDATE contacts 
		SET name = '".$name."',
